...
import { preparePairingRuns } from "../scripts/prepare-pairing.js";
...
test("pairing preparation separates criteria and history while cloning identical isolated databases", t => {
  ...
  for (const mode of ["beginner", "easy", "intermediate", "advanced"]) {
    ...
    for (const excluded of [".git", "node_modules", "dist", "evaluator", "scripts/prepare-pairing.ts", "test/pairing.test.ts"]) {
