...
사용자·Pair·Driver의 반복 대화로 판단을 발견하는 실험입니다. 기능적 완료 기준은
사용자·관찰자 참고 자료로 정의하되 구현 방법은 정하지 않습니다. 비교할 때 어떤
정보를 실제로 읽거나 논의했는지 기록하세요. 앱 테스트는 Navigator 품질이나
개발자의 학습 효과를 측정하지 않습니다.

## 과제와 첫 질문

원본 repository의 [TASK.md](../TASK.md)는 과제 목록과 첫 실습 안내입니다.
각 `tasks/<태스크>/TASK.md`에는 업무 요청, Pair 시작 질문, 실행 안내, 기능적 완료
기준과 범위가 있습니다. `main`에서 확인할 수 있으며 기존 `task/*` 브랜치로 전환할
필요는 없습니다. 완료 기준은 특정 기술·자료구조·알고리즘이나 수정 파일을 지시하지 않습니다.

시작 질문에는 업무 문제만 넣고 기준 문서의 첨부나 읽기를 요청하지 않습니다.
Pair·Driver에게 완료 기준을 자동 전달하지 않으며 Agent가 직접 찾는 것은 허용합니다.
아래 명령으로 내보낼 때도 **짧은 업무 요청과 시작 질문**만 전달합니다. 출력의
...
평가자 폴더의 내용과 경로를 프롬프트에 자동으로 넣지 않습니다. 폴더 분리는
filesystem 접근 차단이 아니며, 넓은 파일·shell 권한으로는 옆 폴더도 읽을 수 있습니다.
Agent가 직접 자료를 찾는 것을 금지하거나 파일 읽기 권한을 별도로 제한하지 않습니다.
...
짧은 요청과 기존 제품 규칙으로 시작합니다. 질문받은 비즈니스 제약은 대화에서
알려주되 전체 기준 목록을 자동으로 덧붙이지 않습니다. 기준을 읽거나 대화에서 합의한
시점을 기록하세요. 공유되지 않은 요구사항을 Agent가 이미 알고 있어야 할 정답으로
채점하지 않습니다. Driver에게 보낼 구현 지시는 사용자가 직접 작성합니다.
