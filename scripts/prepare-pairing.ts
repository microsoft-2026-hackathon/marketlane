...
const scenarios: Record<
  string,
  { assets: string[]; evaluatorOnly?: boolean; followups: string[] }
> = {
  "catalog-cache": {
    assets: [],
    followups: [
  ...
  "stock-reservations": {
    ...
    followups: [
      ...
      "다음: 만료 전후에 예약으로 주문을 시도하고 결과를 확인하세요. 재시작과 예약 수정 실패도 관찰하세요.",
    ],
  },
};

function scenarioFor(task: string) {
  if (!Object.hasOwn(scenarios, task))
    throw new Error(`Choose a task: ${Object.keys(scenarios).join(", ")}`);
...
export function loadPairingTask(task: string): {
  taskText: string;
  ...
  const candidates = [
    ...
    `refs/remotes/origin/task/${task}`,
  ];
  const available = new Set(
    git(["for-each-ref", "--format=%(refname)", ...candidates])
      .trim()
      .split("\n"),
  );
  ...
  if (!taskRef)
    throw new Error(
      `tasks/${task}/TASK.md와 task/${task} 브랜치가 없습니다. git fetch origin을 실행해 주세요.`,
    );
  return {
    ...
    assets: Object.fromEntries(
      scenario.assets.map((filename) => [
        filename,
        execFileSync("git", ["show", `${taskRef}:${filename}`], {
          cwd: sourceRoot,
        }),
      ]),
    ),
...
export function preparePairingRuns(
  task: string,
  outputPath: string,
  taskText: string,
  assets: Record<string, Buffer> = {},
  ...
  if (!brief)
    throw new Error(
      "The task must contain a title and a Business request before other sections.",
    );
  const criteria = /\n## (?:수용 기준|Acceptance criteria)[ \t]*\r?\n([\s\S]*?)(?=\n## |$)/
    .exec(taskText)?.[1]?.trim();
  if (!criteria) throw new Error("The task must contain functional Acceptance criteria.");
  const openingQuestion =
    /\n## Pair 시작 질문[ \t]*\r?\n([\s\S]*?)(?=\n## |$)/
      .exec(taskText)?.[1]
      ?.trim() ??
    "> 이번 업무의 현재 동작을 같이 살펴보자. 어디부터 확인하면 좋을까?";
  ...
  if (isWithin(realpathSync(sourceRoot), requested))
  ...
  if (isWithin(realpathSync(sourceRoot), output))
    throw new Error("Output must be outside the source repository.");
  ...
  const snapshot = files.map((filename) => ({
    filename,
    content: readFileSync(path.join(sourceRoot, filename)),
  }));
  ...
  if (scenario.assets.length) {
    ...
    for (const filename of scenario.assets)
      writeFileSync(
        path.join(evaluator, "inputs", filename),
        assets[filename]!,
      );
  }
  writeFileSync(
    path.join(evaluator, "manifest.json"),
    JSON.stringify(
      {
        task,
        modes,
  ...
  writeFileSync(
    path.join(evaluator, "GUIDE.md"),
    `# ${task}: 평가자 가이드

이 폴더는 두 agent가 연 workspace 밖에 두세요. mode 폴더 하나만 열고,
실험 상위 폴더나 원본 repository는 열지 않습니다. 폴더 분리는 우발적 발견을 줄일 뿐
도구 접근을 차단하지 않습니다. Agent가 자료를 직접 찾는 것은 막지 않습니다.
Agent 프롬프트에 완료 기준이나 이 경로를 자동으로 넣지 마세요.
...
업무 요청과 시작 질문만 제공하며, 완료 기준이나 범위를 Pair·Driver에게 자동 전달하지 않습니다.
Agent가 원본 문서를 직접 찾아 읽는 것은 차단하지 않습니다. 기준을 읽거나 대화 중
확인한 시점은 기록하고, 공유되지 않은 요구사항을 숨은 정답으로 채점하지 마세요.
기능적 완료 기준은 관찰자 참고 자료이며 구현 방식과 trade-off는 열어 두세요.
...
`,
  ...
  for (const mode of modes) {
    ...
    if (!scenario.evaluatorOnly) {
      for (const filename of scenario.assets)
        writeFileSync(path.join(run, filename), assets[filename]!);
    }
    writeFileSync(
      path.join(run, "TASK.md"),
      `${taskText.split("\n")[0]}\n\n## 업무 요청\n\n${brief[1]!.trim()}\n\n기존 제품 규칙 중 이 업무 요청에서 바꾸는 동작 외에는 보존하세요. 미정인 동작과 범위는 사용자와 논의하세요.\n\n## Pair 시작 질문\n\n${openingQuestion}\n`,
    );
    writeFileSync(
      path.join(run, "RUN.md"),
      `# ${task}: ${mode}

VS Code에서 이 폴더만 열고 Wellactually ${mode}의 새 Pair 세션을 시작하세요.
아래 시작 질문으로 논의를 시작합니다. 사용자가 판단하고 Driver 지시를 작성하며,
...
if (
  ...
  try {
    ...
    if (!task || !output || extra.length)
      throw new Error(
        "Usage: npm run pairing:prepare -- <task> <new-output-directory>",
      );
    const { taskText, assets } = loadPairingTask(task);
    console.log(
      `네 mode workspace와 평가자 자료를 생성했습니다: ${preparePairingRuns(task, output, taskText, assets)}`,
    );
  } catch (error) {
